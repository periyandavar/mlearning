            ...
            <ul class="nav">
                ...
                <li class="active"> 
                    <a href="modelquestions.php">
                       ...
                       <p style="font-family:'Charm', cursive; font-size:14px">Model Questions</p>
                    </a>
                </li>
                <li>
            ...
            <div class="container-fluid">
                <div class="navbar-header">
                    <button type="button" class="navbar-toggle">
                     ...
                     <a class="navbar-brand" style="color:#ffffff;font-family: 'IBM Plex Serif', serif;" href="#">Model Questions</a>
                </div>
...
<?php
				  if(isset($_POST['eb1']))
				  {
					...
					$id=$_POST[$i];
					$sql='Update modelquestion set status="'.$st.'" Where id="'.$id.'" and staffcode="'.$_SESSION['value2'].'"'	;
                    (mysqli_query($link,$sql));
				  }
?>
<center><h3>Add Model Questions </h3>
...
<div class="container-fluid"> 
                  ...
                  <div class="row">
                                        ...
                                        <?php 
										 ...
										 { $sbid=$_POST['sub'];}
									      echo'
                                        <div class="col-md-4">
										<div class="form-group"><form id="myform" action="modelquestions.php" method="POST">
	...
	$results=mysqli_query($link,$sql);
while($row=mysqli_fetch_assoc($results))
	{ $sbname=$row['subjectname'];if($sbname!=$sbid)
		{
	  echo "<option value='$sbname'> $sbname ";
		}
	}
...
echo'
                        ...
                        ?>           </div>   <div class="row">
									    <div class="col-md-2">
                                            ...
                                            <div class="form-group"><label> Year & Month </label>
											<input required="required" type="text" class="form-control border-input" name="title" placeholder="Year & Month">
                                             </div>
                                        </div>
										...
										<div class="col-md-4">
										<div class="form-group">
                                             <label>Upload the Question</label>
...
if(isset($_POST['Add']))
	...
	{  
		...
		{
			echo"Error in uploading Model question paper<br>This may be due to Larger file size ";
		}
		else
		{
		//	set_time_limit(0);
...
$file = uniqid().'.pdf';
unlink($sy);
$pdf->Output($_SERVER['DOCUMENT_ROOT'] . '/mlearning/uploads/model_question/'.$syh, 'F');
*/		}
	}
	}
	 if($name=='')
	 {
		 echo"<script>alert('Please select the question paper ')</script>";
	 }
	 elseif($tit=='')
	 {
		 echo"<script>alert('Please Enter the year and month ')</script>";
	 }
	 elseif($sbid=='')
	 {
		 echo"<script>alert('Please select the subject name ')</script>";
	 }
	elseif(!isset($clid)){
 ...
 ($row=mysqli_fetch_assoc($result));
	 $clid=$row['ClassCode'];}
      ...
      $query="insert into modelquestion (StaffCode,ClassCode,SubjectCode,path,Status,title) values ('$id','$clid','$sbid','$sy','y','$tit')";
	     if((mysqli_query($link,$query)))
		 {
		 echo "<h3> Model question paper is added Successfully...!.</h3>";
	     }
		 else
		 {
			 echo"<h3>Error in adding the Model question paper,...!</h3>";
		 }
	 }
}
?>	
<?php
															if(isset($_POST['eb2']))
				  {
					  ...
					  if($_POST['t'.$i]=='t'){
					$id=$_POST[$i];
					$q='select path from modelquestion where id="'.$id.'"';
				    $res=mysqli_query($link,$q);
					($row=mysqli_fetch_assoc($res));
					$file=$row['path'];
					$sql='Delete from photo Where id="'.$id.'"'	;
					$sql='Delete from modelquestion Where id="'.$id.'"'	;
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header">
                                ...
                                <p class="category">Questions upload by us</p>
                            </div>
                            <div class="content table-responsive table-full-width" >
								...
								<?php
									$sql='SELECT  * From modelquestion where StaffCode="'.$_SESSION['value2'].'" limit 0,10 ';
								...
								$q1='select subjectname from subject where subjectcode="'.$row['SubjectCode'].'"';
                                        ...
                                        echo'<tr>
                                        	...
                                        	<td>'.$row['ClassCode'].'</td>
                                        	<td>'.$row['title'].'</td>
											<td><a target="_blank" href="'.$row['path'].'" >click here to view pdf'.'</a></td>';
											if(($row['status'])=='y')
											...
											<td>';
											echo'
<form id="myform" action="modelquestions.php" method="POST">
