<?php
require('php_image_magician.php');
  session_start();
  if($_SESSION['value2']=='')
  {
            ...
            <ul class="nav">
                ...
                <li class="active">
                    <a href="photography.php">
                        <i class="ti-map"></i>
                        <p style="font-family: 'Charm', cursive; font-size:14px">Photography</p>
                    </a>
                </li>
                <li>
            ...
            <div class="container-fluid">
                <div class="navbar-header">
                    <button type="button" class="navbar-toggle">
                     ...
                     <a class="navbar-brand" style="color:#ffffff;font-family: 'IBM Plex Serif', serif;" href="#">Photography</a>
                </div>
...
<?php
				  if(isset($_POST['eb1']))
				  {
					...
					$id=$_POST[$i];
					$sql='Update photo set status="'.$st.'" Where id="'.$id.'" and staffcode="'.$_SESSION['value2'].'"'	;
                    (mysqli_query($link,$sql));
				  }
?>
<center><center><h3>Add Photos </h3>
                                    ...
                                    <div class="row">
										...
										<script>
										 ...
										 { $sbid=$_POST['sub'];}
									      echo'
                                        <div class="col-md-4">
										<div class="form-group"><form id="myform" action="photography.php" method="POST">
...
echo'</select></form>
											...
											<input required="required" type="hidden" name=sbid value="'.$sbid.'">
									    </div>
										</div>
										<div class="col-md-4">
                                            <div class="form-group">
											<label> Class Name </label>
											<select name="clid" class="form-control border-input" >';
											if($sbid!=""){
...
echo'
                        ...
                        ?>             
										...
										<div class="form-group">
                                             <label>Photos</label>
											   <input required="required" id='file' type="file" name="file" accept="image/x-png,image/gif,image/jpeg,image/jpg" style="font-size:14pt;width:270px;"/>   
									    </div>
										</div>
...
<?php
								if(isset($_POST['eb2']))
				  {
					  ...
					  if($_POST['t'.$i]=='t'){
					$id=$_POST[$i];
					$q='select path from photo where id="'.$id.'"';
...
if(isset($_POST['Add']))
	...
	$new=date("Y-m-d H-i-s");
	  ...
	  $sy=$path.$sbj.$new.'.'.$fileextension;	
	  $sy=str_replace(array(' ','<','>',':',"'",'?','*','|','"'),"",$sy);
...
function compress_image($source_url, $destination_url, $quality) {
      ...
      $info = getimagesize($source_url);
		  ...
		  $magicianObj=new imageLib($destination_url);
          $magicianObj->addWatermark('uploads/cust/wm.png','tr',0,16);
          $magicianObj->saveImage($destination_url);
	...
	{  if($fileextension!='jpg' and $fileextension!='jpeg' and $fileextension!='png' and $fileextension!='gif'){  $name=''; }
		else{	$filename = compress_image($_FILES["file"]["tmp_name"], $sy, 80);}
	}
	}
    ...
    elseif(!isset($clid)){
	 ...
	 $clid=$row['ClassCode'];}
      ...
      $query="insert into photo (StaffCode,ClassCode,SubjectCode,path,topic,Status) values ('$id','$clid','$sbid','$filename','$topic','y')";
	     if((mysqli_query($link,$query)))
		 {
		  echo "<h3>photo is added Successfully...!.</h3>";
	     }
		 else{
			 echo"<h3>Error in adding the photo,...!</h3>";
		 }
	 }
}
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header">
                                ...
                                <p class="category">Images upload by us</p>
                            </div>
                            <div class="content table-responsive table-full-width" >
                                <table class="table table-striped">
									...
									<script>
											...
											{
												if(confirm("Are sure you want to delete the Image..?")){
								...
								<?php
									$sql='SELECT  * From photo where StaffCode="'.$_SESSION['value2'].'" limit 0,10 ';
								...
								$q1='select subjectname from subject where subjectcode="'.$row['SubjectCode'].'"';
                                        ...
                                        echo'<tr>
                                        	...
                                        	<td>'.$row['ClassCode'].'</td>
                                        	<td>'.$row['Topic'].'</td>
											<td><a href="'.$row['path'].'" target="white" ><img width="340" height="200" src="'.$row['path'].'" >'.'</img></a></td>';
											if(($row['status'])=='y')
