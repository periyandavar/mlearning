...
<form action="view_modelquestions.php" method="POST"><h5>Search For Part I Papers</h5>
...
<form action="view_modelquestions.php" method="POST"><h5>Search For Part IV Papers</h5>
<table border="0" width="850" height="70">
<tr><td style="font-size:14pt;width:150px;">Subject Name</td><td><select name="sbid" style="font-size:14pt;width:450px;"><option>
<?php
...
<form action="view_modelquestions.php" method="POST">
<table border="0" width="850" height="70">
<tr><td style="font-size:14pt;width:150px;">Subject Name</td><td><select name="sbid" style="font-size:14pt;width:450px;"><option>
<?php
...
if(isset($_POST['Go']))
    {
	 ...
	 else	 
	 {
      ...
      $sql='SELECT path From modelquestion where SubjectCode="'.$name3.'"'.'and StaffCode ="'.$sname.'"and ClassCode="'.$name.'"' ;
	  $rt=mysql_query($sql,$link);
	  $num=mysql_num_rows($rt);	  
   	  for($j=0;$j<$num;$j++)
	  {
	  $a=mysql_result($rt,$j,'path');
	  echo "<h4>Model Question Paper - ".($j+1)."<br><a target='white' href='".$a."' ><br>".$a."<br></a><br></h4>";
	  $flag++;
	  }
	  }
	  if($flag==0)
	  {
		  echo"<script>alert('Model questions are not yet uploaded for this unit,..!')</script>";
	  }
	 }else{$sql='SELECT path From modelquestion where SubjectCode="'.$name3.'" and status="y"' ;
	  $rt=mysql_query($sql,$link);
	  $num=mysql_num_rows($rt);
	  ...
	  if($flag==0)
	  {
		  echo"<script>alert('Notes are not yet uploaded for this unit,..!')</script>";
	 }}
	 }
	} 
	?>
            ...
            echo'  <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header">
                                <h4 class="title">Model Questions</h4>
                                <p class="category">Available Model Question papers </p>
                            </div>
                            <div class="content table-responsive table-full-width">
                                <table class="table table-striped">
                                    <thead>
                                        <th>Model Questions </th>
                                    	
                                    </thead>
                                    <tbody>';
									for($j=0;$j<$num_rows1;$j++)
									{
								$sql='SELECT title,path From modelquestion where classcode="'.$gg.'"and staffcode="'.mysql_result($result1,$j,'staffcode').'"and subjectcode="'.mysql_result($result1,$j,'papercode').'" limit 0,10';
                        ...
                        <div class="alert alert-success">
                                    ...
                                    <span><b> '.mysql_result($result7,0,'subjectname').$row['title'] .'<br> <color:orange"><a target="white" href="'.$row['path'].'"> <i class="ti-bell"></i>'.$row['path'].'"</a></b></span>
                     </div></div>   </td>
